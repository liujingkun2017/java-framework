...
import org.liujk.java.framework.base.utils.lang.beans.collection.ArrayQueue;
...
public class IOUtils {
    ...
    /**
     * 在虚拟机终止时，请求删除此路径名表示的文件或目录。仅在虚拟机正常终止时，才会试图执行删除操作，这在 Java Language Specification 中已定义。
     *
     * @param filePath 要删除文件或目录的路径。
     *
     ...
     * FileOperateException 如果抽象路径名不表示一个目录，或者发生错误。
     */
    ...
    /**
     * 返回表示路径名所表示目录中的文件和目录的抽象路径名数组，这些路径名满足特定过滤器。此方法的行为与 {@link #fileArray(String)}
     * 方法相同，除了所返回数组中的路径名必须满足过滤器。如果给定 filter 为 null，则接受所有路径名。否则，当且仅当在路径名上调用过滤器的
     * {@link FileFilter#accept(File)} 方法返回 true 时，该路径名才满足过滤器。
     *
     * @param filePath   要查找所有文件的目录路径名。
    ...
    /**
     * 返回表示路径名所表示目录中的文件和目录的抽象路径名{@link List}，这些路径名满足特定过滤器。此方法的行为与 {@link #fileArray(String)}
     * 方法相同，除了所返回{@link List}中的路径名必须满足过滤器。如果给定 filter 为 null，则接受所有路径名。否则，当且仅当在路径名上调用过滤器的
     * {@link FileFilter#accept(File)} 方法返回 true 时，该路径名才满足过滤器。
     *
     * @param filePath   要查找所有文件的目录路径名。
     * @param fileFilter 文件过滤器。
    ...
    /**
     * 返回表示路径名所表示目录中的文件和目录的抽象路径名数组，这些路径名满足特定过滤器。此方法的行为与 {@link #fileArray(String)}
     * 方法相同，除了所返回数组中的路径名必须满足过滤器。如果给定 filter 为 null，则接受所有路径名。否则，当且仅当在此抽象路径名和它所表示的目录中的文件名或目录名上调用过滤器的
     ...
     *
     * @param filePath 路径名。
     * @param time     最后一次修改的时间，用该时间与历元（1970 年 1 月 1 日，00:00:00 GMT）的时间差来计算（以毫秒为单位）。
     *
     ...
     * @param filePath 路径名。
     *
     * @return 表示文件最后一次被修改的时间 Date , 如果该文件不存在，或是发生 I/O 错误，则 Date.getTime() 返回 0L。
     */
     ...
     * @param filePath 路径名。
     *
     * @return 表示文件最后一次被修改的时间 Calendar , 如果该文件不存在，或是发生 I/O 错误，则 Calendar.getTimeInMillis() 返回 0L。
     */
    ...
    /**
     * 测试路径名指定的文件是否是一个隐藏文件。隐藏 的具体定义与系统有关。在 UNIX 系统上，如果文件名以句点字符 ('.') 开头，则认为该文件被隐藏。在 Microsoft
     * Windows 系统上，如果在文件系统中文件被标记为隐藏，则认为该文件被隐藏。
     *
     * @param filePath 路径名。
     *
    ...
    /**
     * 重新命名路径名表示的文件。 此方法行为的许多方面都是与平台有关的：重命名操作无法将一个文件从一个文件系统移动到另一个文件系统，该操作可能不是原子的，
    ...
    /**
     * 标记路径名指定的文件或目录，以便只可对其进行读操作。在调用此方法后，可以保证在被删除或被标记为允许写访问之前，文件或目录不会发生更改。 是否可以删除某个只读文件或目录则取决于基础系统。
     *
     * @param filePath 路径名。
     *
     * @return 当且仅当该操作成功时，返回 true；否则返回 false。
     ...
     * @throws IOException 发生 I/O 错误时。
     */
    public static int copy (InputStream in, OutputStream out, boolean closeIn, boolean closeOut)
            throws IOException {
        try {
            int byteCount = 0;
    ...
    private static int copy (File in, File out, int byteCount, Queue<File> copyFileQueue)
            throws IOException {
        ...
        if (in.isDirectory ()) { // 为目录
            ...
            byteCount = byteCount + copy (new BufferedInputStream (new FileInputStream (in)),
                    new BufferedOutputStream (new FileOutputStream (out)), true, true);
            if (copyFileQueue != null) {
                copyFileQueue.offer (out);
            }
            return byteCount;
