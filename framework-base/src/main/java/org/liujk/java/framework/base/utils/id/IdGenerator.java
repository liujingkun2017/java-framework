...
public class IdGenerator {
    ...
    /**
     * 构造函数 工作ID (0~31) 数据中心ID (0~31)
     */
    public IdGenerator() {
        this.datacenterId = (this.datacenterId < 0 ? RandomUtils.nextLong (0, maxDatacenterId) : this.datacenterId) &
                maxDatacenterId;
        this.workerId = (this.workerId < 0 ? RandomUtils.nextLong (0, maxWorkerId) : this.workerId) & maxWorkerId;
    }
    ...
    public synchronized long nextId (long workerId) {
        ...
        if (timestamp < lastTimestamp) {
            throw new FormatRuntimeException (String.format (
        ...
        String strId1 = strId.substring (0, 1);
        String strId2 = strId.substring (4);
        if (this.isOr) {
            strId1 = strId;
            strId2 = "";
        }
        return Long.parseLong (strId1 + strId2);
    }
